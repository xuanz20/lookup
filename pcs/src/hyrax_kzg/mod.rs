...
mod tests {
    ...
    use ark_ec::{bls12::G1Prepared, pairing};
    ...
    fn test_single_helper<R: Rng>(
        ...
        let (ck, vk) = HyraxKzgPCS::trim(srs);
        let commit = HyraxKzgPCS::commit(&ck, eval);
        let point: Vec<_> = (0..num_vars).map(|_| F::rand(rng)).collect();
        let (proof, value) = HyraxKzgPCS::open(&ck, eval, &point);
        assert!(HyraxKzgPCS::verify(&vk, &commit, &point, &proof, value));
    ...
    fn test_single_commit() {
        ...
        let srs  = HyraxKzgPCS::<E>::gen_srs(&mut rng, 10);
